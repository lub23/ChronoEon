...
use sqlx::{migrate::MigrateDatabase, pool::PoolOptions, Column, Executor, Pool, Row};
#[cfg(any(feature = "sqlite", feature = "mysql", feature = "postgres"))]
use tauri::Manager;
    ...
    pub(crate) async fn connect<R: Runtime>(
            ...
            .0
        {
            ...
            "sqlite" => {
                ...
                // ChronoEon issues explicit BEGIN/COMMIT statements through this
                // facade. A one-connection SQLite pool pins those statements to
                // the same connection; SQLx's default 10-connection pool can
                // return BEGIN on one and the next statement on another, which
                // surfaces to users as "database is locked".
                Ok(Self::Sqlite(PoolOptions::new().max_connections(1).connect(conn_url).await?))
            }
            #[cfg(feature = "mysql")]
            "mysql" => {
